class Groups::GamesController < GroupsControllerBase
  before_action :set_game, only: %i[ show add_player remove_player set_player_team ]
  ...
  def set_player_team
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.replace("game_players_selector_#{@game.group.id}", partial: 'groups/games/players_selector', locals: { group: @group, game: @game })
      end
      format.json { head :ok }
    end
  end
